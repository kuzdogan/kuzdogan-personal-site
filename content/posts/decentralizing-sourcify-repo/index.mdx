...
I don't like this structure for several reasons:

1. The reasoning for chapters is to make the user download more data then they need and pin the data.
   1. I don't think the decentralization of data is going to be a problem if there are multiple publishers, pinning services, and volunteers.
   2. A contract should be directly accessible with its chain+address identifier. Consider a block explorer frontend wanting to pull the verified contract with IPFS ([Otterscan does this](https://sepolia.otterscan.io/address/0x2738d13E81e30bC615766A0410e7cF199FD59A83/contract) if you choose "Resolve IPNS" from top-right). The frontend should not be required to download all the chapters on all volumes to serve the source files.
   3. I can see the number of volumes getting unreasonably high at some point, as the user need to download all the chapters of all volumes.
   4. There is no client like `chifra` to "consume" the verified contract. The files being downloaded are just text files. The user just want to see a single contract, likely on a web interface. How can we make them run an ipfs node and pin the files?

Sourcify's repository is structured as follows:
...
The IPNS resolves to the root CID of the repository. This way one can directly access the contract. Now in this case one only trusts Sourcify but if there's a root hash `Qmf9BvoWcX8NPSwXWwLBDTvbhSoeDPmxvfANS7JR5c1Mq4` manifested by multiple publishers you trust you can do
https://ipfs.io/ipfs/Qmf9BvoWcX8NPSwXWwLBDTvbhSoeDPmxvfANS7JR5c1Mq4/contracts/full_match/11155111/0x2738d13E81e30bC615766A0410e7cF199FD59A83/

That brings me to thinking, can we get away with just (again blockchain similarities) publishing the volumes (a block), that contains each new verified contract (transactions) and new root hash (chain head block hash)?

Sourcify receives 1000 new verified contracts, publishes the new contracts and the root IPFS hash, Blockscout takes the new contracts, verifies them, places them in the repo, publishes their root IPFS hash and hopefully we'll have the same hash.
